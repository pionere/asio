...
template <typename SyncReadStream, typename DynamicBuffer>
std::size_t read_until(SyncReadStream& s, DynamicBuffer&& buffers,
    const boost::regex& expr, asio::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    if (regex_search(start_pos, end, match_results,
          expr, boost::match_default | boost::match_partial))
    {
...
template <template <typename, typename> class Associator,
    typename AsyncReadStream, typename DynamicBuffer,
...
async_read_until(AsyncReadStream& s,
    DynamicBuffer&& buffers, char delim, ReadHandler&& handler)
{
...
ASIO_INITFN_AUTO_RESULT_TYPE(ReadHandler,
    void (asio::error_code, std::size_t))
async_read_until(AsyncReadStream& s, DynamicBuffer&& buffers,
    ASIO_STRING_VIEW_PARAM delim, ReadHandler&& handler)
{
  // If you get an error on the following line it means that your handler does
  // not meet the documented type requirements for a ReadHandler.
  ASIO_READ_HANDLER_CHECK(ReadHandler, handler) type_check;
        ...
        {
          {
            ...
            bool match = regex_search(start_pos, end,
                match_results, expr_, boost::match_default | boost::match_partial);
